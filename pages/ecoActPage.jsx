  ...
  return (
    <div data-theme={dark ? "dark" : "light"} className="min-h-screen">
      ...
      <main className="pt-[40px] flex flex-col items-center h-400 px-4">
        {/* Contoh konten */}
        <h1 className="text-[48px] top-0 font-bold mb-4">EcoAct-Daftar Kegiatan Volunter</h1>
        <label className="input w-[750px] rounded-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-35 mt-20 gap-y-10 max-w-fit mx-auto">
          <div className="bg-white rounded-2xl shadow-xl p-10 w-full  mx-auto">
            <div className="space-y-5">
              {Object.entries(data).map(([label, value]) => (
                <div key={label} className="flex">

                  <span
                    className="font-semibold text-green-700 text-2xl text-left"
                    style={{ minWidth: `${maxLabelWidth}ch`, display: "inline-block" }}
                  >
                    {label}:
                  </span>
                  <span className="text-gray-800 text-2xl flex-grow">{value}</span>
                </div>
              ))}
            </div>

            <div className="mt-6 flex flex-col items-center gap-3">
              <button className="bg-lime-500 text-white font-semibold px-8 mb-4 py-4 text-2xl rounded-full w-full hover:bg-lime-600 transition">
                Daftar
              </button>
              <button className="text-lime-600 border border-lime-500 px-6 py-4 text-2xl rounded-full w-full font-medium hover:bg-lime-50 transition">
                Lihat Selengkapnya
              </button>
            </div>
          </div>
